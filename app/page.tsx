...
function Dashboard({ projects, onAdd, onOpen, onEdit, onDelete }: { projects: Project[]; onAdd: () => void; onOpen: (p: Project) => void; onEdit: (p: Project) => void; onDelete: (p: Project) => void }) {
  ...
  return <>
    ...
    <section className="dashboard-columns">
      <div className="panel"><div className="panel-title"><h2>项目管理</h2><span>共 {projects.length} 个项目</span></div><div className="project-scroll">
      ...
      <div className="panel"><div className="panel-title"><h2>跨项目重点问题</h2></div><table><thead><tr><th>项目</th><th>问题</th><th>数量</th><th>状态</th></tr></thead><tbody><tr><td>x.pink</td><td>充值成功未到账</td><td>8</td><td><span className="pill red">高</span></td></tr><tr><td>KISSLY</td><td>未查到支付记录</td><td>5</td><td><span className="pill red">高</span></td></tr><tr><td>x.pink</td><td>生成结果不满意</td><td>14</td><td><span className="pill amber">中</span></td></tr></tbody></table><div className="warning">项目数据相互隔离；切换项目后，邮件、账号和后台查询结果全部重新切换。</div></div>
    </section>
  </>;
}
...
function ReviewCenter({ tab, onTab, project, reply, notify }: { tab: "mail" | "gems"; onTab: (t: "mail" | "gems") => void; project: Project; reply: string; notify: (s: string) => void }) {
  return <><Header title="人工审核中心" subtitle="邮件发送和宝石赔偿分别确认，系统不会自动执行" /><div className="tabs"><button className={tab === "mail" ? "active" : ""} onClick={() => onTab("mail")}>邮件发送审核 <span>5</span></button><button className={tab === "gems" ? "active" : ""} onClick={() => onTab("gems")}>宝石赔偿审核 <span>4</span></button></div><div className="review-layout"><section className="panel review-list"><h2>{tab === "mail" ? "待审核回复" : "待审核赔偿"}</h2><button className="review-item selected"><strong>{tab === "mail" ? "x.pink Payment issue" : "x.pink Generation result"}</strong><span>{tab === "mail" ? "user@example.com" : "maria@example.com"}</span><small>{tab === "mail" ? "等待发送审核" : `建议赔偿 ${project.defaultGems} ${project.gems}`}</small></button><button className="review-item"><strong>KISSLY Credits not received</strong><span>kissly.user@example.com</span><small>等待人工确认</small></button></section><section className="panel review-detail"><div className="tag-row"><span className="pill green">{project.name}</span><span className="pill red">{tab === "mail" ? "支付问题" : "产品投诉"}</span></div><h2>{tab === "mail" ? "发送前确认" : "赔偿前确认"}</h2><dl><dt>项目与客户账号</dt><dd>{project.name} · user@example.com</dd><dt>原始诉求</dt><dd>客户表示已支付 89 美元，但没有收到 5000 Gems。</dd><dt>后台查询结果</dt><dd className="success-result">已查到记录：$89 · 5000 Gems · 支付成功</dd>{tab === "mail" ? <><dt>将要发送的完整邮件</dt><dd>{reply}</dd></> : <><dt>赔偿数量和原因</dt><dd>{project.defaultGems} {project.gems} · 产品体验补偿</dd></>}</dl><div className="button-row end"><button className="danger" onClick={() => notify("审核已拒绝，未执行任何操作")}>拒绝</button><button onClick={() => notify("已退回修改")}>退回修改</button><button className="primary" onClick={() => notify(tab === "mail" ? "已批准发送；等待邮箱连接后执行" : "已批准赔偿；等待后台连接后执行")}>批准{tab === "mail" ? "发送" : "赔偿"}</button></div><div className="warning">批准只针对当前操作；发信与赔偿不能使用同一个按钮执行。</div></section></div></>;
}
